...
public class UIDropDown: UIControl {
    ...
    open var extraWidth:CGFloat = 170
    open var multipleSelectionOption: Bool = false
    open var adjustsFontSizeToFitWidthValue : Bool = true
    fileprivate var arrow: Arrow!
    fileprivate var table: UITableView!
    
    public fileprivate(set) var selectedIndex: Int?
    public fileprivate(set) var selectedIndexArray: [Int] = []
    public var options = [String]()
    public var hideOptionsWhenSelect = false
    public var placeholder: String! {
        didSet {
            title.text = placeholder
            title.adjustsFontSizeToFitWidth = adjustsFontSizeToFitWidthValue
        }
    }
    ...
    fileprivate var privatedidSelectMultiple: ([String], [Int]) -> () = {options, indices in }
    ...
    public func didSelectMultiple(completion: @escaping (_ option: [String], _ index: [Int]) -> ()) {
        privatedidSelectMultiple = completion
    }
...
extension UIDropDown: UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if multipleSelectionOption {
            cell!.accessoryType = (selectedIndexArray.contains(indexPath.row)) ? .checkmark : .none
        } else {
...
extension UIDropDown: UITableViewDelegate {
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if multipleSelectionOption {
            if  selectedIndexArray.contains((indexPath as NSIndexPath).row) {
                let ind = selectedIndexArray.index(of: (indexPath as NSIndexPath).row)
                selectedIndexArray.remove(at: ind!)
            } else {
                selectedIndexArray.append((indexPath as NSIndexPath).row)
            }
        } else {
        ...
        if multipleSelectionOption {
            ...
            for index in selectedIndexArray ?? [] {
                optionsArray.append(self.options[index])
            }
            privatedidSelectMultiple(optionsArray,selectedIndexArray ?? [])
        } else {
